...
def build_app():
    ...
    def recommend():
        ...
        if " by " not in user_text:
            ...
            try:
                rec_id_list = retrieve_recs(
                    user_track_id, user_track_features, user_year)
            except:
                message = "There is something wrong. Come try again at a later time."
                return render_template('index.html', message=message)

            rt1_id = rec_id_list.loc[0]['id']
            rt1_name = rec_id_list.loc[0]['name']
            rt1_artist = rec_id_list.loc[0]['artists']

            rt2_id = rec_id_list.loc[1]['id']
            rt2_name = rec_id_list.loc[1]['name']
            rt2_artist = rec_id_list.loc[1]['artists']

            rt3_id = rec_id_list.loc[2]['id']
            rt3_name = rec_id_list.loc[2]['name']
            rt3_artist = rec_id_list.loc[2]['artists']

            rt4_id = rec_id_list.loc[3]['id']
            rt4_name = rec_id_list.loc[3]['name']
            rt4_artist = rec_id_list.loc[3]['artists']

            rt5_id = rec_id_list.loc[4]['id']
            rt5_name = rec_id_list.loc[4]['name']
            rt5_artist = rec_id_list.loc[4]['artists']
            ...
            for i in range(len(rec_id_list)):
                tup = (rec_id_list.loc[i]['name'],
                       rec_id_list.loc[i]['danceability'])
                data1.append(tup)
            ...
            for i in range(len(rec_id_list)):
                tup = (rec_id_list.loc[i]['name'],
                       rec_id_list.loc[i]['acousticness'])
                data7.append(tup)
            ...
            for i in range(len(rec_id_list)):
                tup = (rec_id_list.loc[i]['name'],
                       rec_id_list.loc[i]['instrumentalness'])
                data8.append(tup)
            ...
            for i in range(len(rec_id_list)):
                tup = (rec_id_list.loc[i]['name'],
                       rec_id_list.loc[i]['time_signature'])
                data12.append(tup)
